                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.leads.update", [$lead->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('mobile') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="mobile" id="mobile" value="{{ old('mobile', $lead->mobile) }}">
                        ...
                        <div class="form-group {{ $errors->has('gender') ? 'has-error' : '' }}">
                            <label>{{ trans('cruds.lead.fields.gender') }}</label>
                            <select class="form-control" name="gender" id="gender">
                                <option value disabled {{ old('gender', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                                @foreach(App\Lead::GENDER_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('gender', $lead->gender) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('date_of_birthday') ? 'has-error' : '' }}">
                            <label for="date_of_birthday">{{ trans('cruds.lead.fields.date_of_birthday') }}</label>
                            <input class="form-control date" type="text" name="date_of_birthday" id="date_of_birthday" value="{{ old('date_of_birthday', $lead->date_of_birthday) }}">
                            @if($errors->has('date_of_birthday'))
                                <span class="help-block" role="alert">{{ $errors->first('date_of_birthday') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.lead.fields.date_of_birthday_helper') }}</span>
                        </div>
                        <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }}">
                            <label for="address">{{ trans('cruds.lead.fields.address') }}</label>
                            <textarea class="form-control" name="address" id="address">{{ old('address', $lead->address) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="description" id="description">{!! old('description', $lead->description) !!}</textarea>
