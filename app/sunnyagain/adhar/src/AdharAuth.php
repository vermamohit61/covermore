<?php
namespace App\Sunnyagain\Adhar\Src;
...
{ 
	...
	**/
     ...
     * 
	**/
	private $_authServer = 'http://auth.uidai.gov.in/';
	...
	**/
	...
	private function getAuthEndPoint(){
		...
		{
			$this->_authEndPoint = $this->_authServer . $this->getApiVersion() . "/public/" . $this->_uid[0] . "/" . $this->_uid[1] . "/" . $this->getAsaLicenceKey();
		}
    ...
    private function createPidBlock(){
    	...
    	$xml .= 'ts="'.$this->getTs() .'" ';
    	$xml .= 'xmlns:ns2="http://www.uidai.gov.in/authentication/uid-auth-request-data/1.0"><ns2:Demo><ns2:Pid ';
    	$xml .= 'ms="'.$this->getMs().'" ';
    	$xml .= 'mv="'.$this->getMv().'" ';
	...
	{ 
      //echo $this->getPublicCertPath();exit;
      $randkey = $this->generateRandomString();
      $_encryptBySessionKey = $randkey;
	...
	{
      //print_r($arrguments);exit;
		if(!empty($arrguments))
		{
			$this->setOptions($arrguments);
		}
        
        //echo $this->getAuthEndPoint();exit;
